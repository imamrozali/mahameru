...
const Sidebar: React.FC<SidebarProps> = ({ children, variant = "default" }) => {
  ...
  return (
    <div className="flex h-screen">
      ...
      <motion.div
        className={cn(
          "relative bg-gray-800 text-white z-[1000]",
      ...
      >
        <div
          className={cn(variant === "bento" && "relative bg-gray-800 top-0")}
        >
          <button
          ...
          <ul className="mt-10 space-y-2">
            ...
            <li
              ...
              //   onMouseLeave={() => !expanded && setOpen(false)}
            >
              <button
              ...
              <motion.ul
                ...
                className={`${open ? "block" : "hidden"} ${
                  expanded ? "mt-1" : "absolute left-full top-0 ml-1"
      ...
      {/* Main Content */}
      <div className="w-full fixed h-screen">{children}</div>
    </div>
  );
};
